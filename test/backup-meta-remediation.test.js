...
test("scheduler, lock, filename, and retention meta-remediation matrix", async (t) => {
  ...
  await t.test("stale tombstone recovery", () => { reset(); const filename = "rootark-backup-2020-01-01-00-00-00.zip"; const tombstone = path.join(backupService.BACKUPS_DIR, `${filename}.retention-tombstone`); fs.writeFileSync(tombstone, "archive"); backupService.recoverRetentionTombstones(); assert.equal(fs.existsSync(path.join(backupService.BACKUPS_DIR, filename)), true); });
  ...
  await t.test("interrupted-retention restart", () => { reset(); const filename = "rootark-backup-2020-01-01-00-00-00.zip"; fs.writeFileSync(path.join(backupService.BACKUPS_DIR, `${filename}.retention-tombstone`), "archive"); backupService.recoverRetentionTombstones(); assert.equal(fs.readFileSync(path.join(backupService.BACKUPS_DIR, filename), "utf8"), "archive"); });
});
