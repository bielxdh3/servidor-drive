...
- Permission-removal and token-expiry cases lack focused coverage. Current code increments `sessionVersion` for password, role, permission, and disabled changes, but not every mutation path has runtime proof.
...
| Current users revoke stale HTTP sessions | Focused test rejects disabled and version-mismatched users | Permission-removal route proof | Change one permission, then call its protected HTTP route with the old cookie |
