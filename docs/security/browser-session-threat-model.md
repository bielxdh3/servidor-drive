...
- Each authenticated HTTP request resolves the current server-side user, so disabled, deleted, and session-revoked users fail on their next request. Before processing the next authenticated WebSocket message or sending the next authenticated realtime event, the server rejects a missing or elapsed JWT expiry, reloads the current user, and compares enabled/deleted state and `sessionVersion` with the socket identity.
...
- Focused regression tests prove that removing `manageUsers` through `PUT /users/:username` increments `sessionVersion`, revokes the old HTTP browser session, and closes an already-open WebSocket with `1008` and no post-revocation `pong` on its next `ping`. They also use a real server and `ws` client to prove that an already-open short-lived JWT socket receives no post-expiry `pong` and closes with `1008` and `Sessao expirada` on its next `ping`. The command is `node --test test/auth-security.test.js`; other session-version mutation paths still lack focused coverage.
...
| WebSocket upgrade uses current cookie session and Origin | Origin helper plus real-WebSocket permission-removal and JWT-expiry regressions | Other session-version mutation paths | Open a socket, revoke another session-affecting field, then verify the required post-revocation behavior |
