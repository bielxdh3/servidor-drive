...
- `compartment_id`, `object_id`, `version_id`, `key_ref`, `sender_key_id`, `recipient_key_id`, `replay_id`, and `idempotency_key`: definite byte strings, each `1..128` bytes.
...
The exact authenticated bytes are:

1. `aad_map = {"profile":"rootark-zk-1/aad/v1","suite":"rootark-zk-1","envelope_version":envelope_version,"compartment_id":compartment_id,"epoch":epoch,"purpose":purpose,"object_id":object_id,"version_id":version_id,"key_ref":key_ref,"manifest_digest":SHA-256(manifest_bytes)}`; `aad = deterministic_cbor(aad_map)`.
2. `info_map = {"profile":"rootark-zk-1/hpke-info/v1","suite":"rootark-zk-1","envelope_version":envelope_version,"compartment_id":compartment_id,"epoch":epoch,"purpose":purpose,"object_id":object_id,"version_id":version_id,"key_ref":key_ref,"sender_key_id":sender_key_id,"recipient_key_id":recipient_key_id,"manifest_digest":SHA-256(manifest_bytes)}`; `info = ASCII("Root.ark/zk-1/hpke-info/v1") || 0x00 || deterministic_cbor(info_map)`.
3. `hpke_info_digest = SHA-256(info)`. The exact `info` and `aad` byte strings are passed to HPKE/AEAD; no JSON or implementation-defined re-encoding is permitted.
...
The manifest map has exactly these fields: `type`=`rootark-authorization-manifest-v1`, `suite`, `envelope_version`, `compartment_id`, `epoch`, `purpose`, `sender_key_id`, `recipient_key_id`, `object_id`, `version_id`, `key_ref`, `expiry`, `replay_id`, `idempotency_key`, `hpke_enc`, `hpke_info_digest`, `wrapped_key_digest`, and `ciphertext_digest`.
...
| `@noble/curves`, `@noble/ciphers`, `@noble/hashes` | 2.3.0 each; exact integrity values were not present in the read-only evidence packet | [noble-curves](https://github.com/paulmillr/noble-curves) and related upstream sources; primitive-level audit statements do not close Root.ark integration, package supply chain, reproducibility, maintenance, or JS zeroization. | `REQUIRED_TECHNICAL_CORRECTION` |
| `argon2-browser` | 1.18.0; integrity not present in the read-only evidence packet | [npm argon2-browser](https://www.npmjs.com/package/argon2-browser); browser/WASM candidate is stale for this review and does not close Node, vector, formal-audit, or zeroization provenance. | `BLOCKING_PHASE_8` |
| `argon2id` | 1.0.1; integrity not present in the read-only evidence packet | [npm argon2id](https://www.npmjs.com/package/argon2id); JavaScript runtime, maintenance, vectors, audit, memory handling, and zeroization require verification. | `BLOCKING_PHASE_8` |
| `@node-rs/argon2` | 2.1.0; integrity not present in the read-only evidence packet | [npm @node-rs/argon2](https://www.npmjs.com/package/@node-rs/argon2); Node-native candidate does not close browser support; native backend, platform, audit, reproducibility, maintenance, and zeroization require review. | `BLOCKING_PHASE_8` |
