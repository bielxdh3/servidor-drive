...
- `[IMPLEMENTED-UNVERIFIED]` Add tests for disabled users, removed permissions, expiry, revocation, and realtime authentication; `test/auth-security.test.js` proves that removing `manageUsers` through `PUT /users/:username` closes an already-open authenticated WebSocket with `1008` on its next `ping`, without a second `pong`, and that an already-open JWT-expired socket closes with `1008` and `Sessao expirada` before its next `ping` is processed (`node --test test/auth-security.test.js`). Other session-version mutation paths remain unverified.
