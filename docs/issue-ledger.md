...
| 9 | Zero-knowledge implementation program | `NOT_STARTED` | Current server-readable encryption, previews, scanning, WebDAV, sync, and backup remain legacy behavior; no crypto implementation or implementation-only scaffolding was added | Phase 9+ vectors, interoperability, runtime implementation, migration, and release acceptance remain open; migration/runtime stay out of scope for this foundation | Begin only the separately authorized bounded Phase 9 foundation; do not claim runtime, migration, or release readiness |
