...
| #9 | `implemented-locally-unverified` | Phase 10 TOTP/2FA is implemented and locally validated in the bounded scope documented at `docs/security/phase-10-totp.md`; remote issue state, browser/production/release acceptance, and independent final review remain open. |
...
| 9 | Zero-knowledge implementation program | `PARTIAL` | `PHASE_9_BOUNDED_FOUNDATION_ACCEPTED`: local `rootark-zk-1` vectors and fail-closed tests pass after dependency installation; current server-readable storage and full migration/runtime/release behavior remain outside the bounded foundation | Reproducibility/provenance, migration/runtime, provider/browser/CI/production, and independent review remain open | Preserve the bounded foundation and hand off full implementation only through a separately scoped phase |
| 10 | Issue #9 2FA/TOTP | `PARTIAL` | TOTP enrollment, challenge, recovery, disable/reset, policy, migration, session/realtime fencing, and disposable HTTP evidence are recorded in `docs/security/phase-10-totp.md` | Browser, remote CI, production, release, and independent final review remain open | Keep remote issue/publication state untouched; complete the next authorized review gate |
